...
### US-003: 조회수/판매량 기반 인기 상품 조회
**As a** 고객
**I want to** 조회수와 판매량을 종합한 인기 상품을 보고 싶다
**So that** 실제로 인기 있는 상품을 확인할 수 있다

**인수 조건**
- [ ] 조회수와 판매량에 가중치를 적용한 인기도 점수로 Top 5 상품이 조회된다
- [ ] 상품명, 가격, 재고, 조회수, 판매량, 인기도 점수가 표시된다
- [ ] 인기도 점수가 높은 순서대로 정렬된다
- [ ] 인기도 점수 = (조회수 × 0.1) + (판매량 × 0.9)

**테스트 시나리오**

```gherkin
Given 여러 상품들이 다양한 조회수와 판매량을 가지고 있고
When 인기 상품 Top 5를 조회하면
Then 인기도 점수가 높은 순서대로 5개 상품이 반환된다 And 각 상품의 조회수, 판매량, 인기도 점수가 포함되어 있다

Given 상품 A(조회수 100, 판매량 50), 상품 B(조회수 200, 판매량 30), 상품 C(조회수 50, 판매량 100)가 있을 때
When 인기 상품을 조회하면
Then 상품 C(점수 95), 상품 A(점수 55), 상품 B(점수 47) 순서로 반환된다
```
